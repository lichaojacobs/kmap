...
$('#secondstep .point span:eq(1)').append(belongcollege+' · '+selectyear+' · '+selectcourse);
$('#thirdstep .point span:eq(1)').append(belongcollege+' · '+selectyear+' · '+selectcourse);
}
else
{
alert("请先选择课程！");
        ...
        success:function(data,status){
            if(data.pointForward!=null||data.pointBehind!=null)
            {
                ...
                if(data.pointForward!=null)
                {
                    ...
                    {
                        pointforward.add(new Option(data.pointForward[i].name,datapointForward[i].id));
                    }
                }
                if(data.pointBehind!=null)
                {
                    ...
                    {
                        pointbehind.add(new Option(data.pointBehind[i].name,data.pointBehind[i].id));
                    }
                }
          ...
          <div class="newsearch">
              ...
              <div id="thirdstep" class="vis">
                ...
                <div class="queryform" style="margin-left: 80px; height:460px">
				  ...
				  <input type="text" id="pointnameun" name="pointnameun" class="form-control" disabled="disabled" style="display:inline-block; width:60%; margin-left:16px" />
                  </div>

                  <div class="pointspace">
                      <label for="pointlevel">知识点级别</label>
                      <select id="pointlevel" style="width: 60%" name="pointlevel" class="drop" onchange="loadByLevel(this)">
                  ...
                  <button class="btn-primary btn" onclick="GenerateDetailView()">预览</button>
                </div>
                <!--right-->
                </div>
              </div>
...
<script type="text/javascript">
    (function () {
        ...
        var GenerateOverview = function (title) {
            ...
            _html += '<a id="mb_ico">x</a><div><canvas id="sitemap" width="100%" height="500"></canvas></div><div id="mb_btnbox">';
...
<script type="text/javascript">
    ...
    function AddToDatabase(){
        ...
        $.ajax({
            ...
            data:{"course":selectcourse,"pointname":pointname,"pointdescription":pointdescription,"pointforward":pointforward,"pointbehind":pointbehind,"academy":belongcollege},
            contentType: "application/x-www-form-urlencoded; charset=UTF-8",
            dataType:"json",
            type: "POST",
            success:function(data,status){
                ...
                if(status=="success")
                {
                    if(data.pointForward!=null||data.pointBehind!=null)
                    {
                        ...
                        if(data.pointForward!=null)
                        {
                            for(var i=0;i<data.pointForward.length;i++)
                            {
                                pointforward.add(new Option(data.pointForward[i].name,data.pointForward[i].id));
                            }
                        }
                        if(data.pointBehind!=null)
                        {
